...
class WifiCSIDataset(Dataset):
    def __init__(self, file_path, recreate = False):
        if os.path.exists(file_path.replace('.csv', '_labels.pt')) and os.path.exists(file_path.replace('.csv', '_data.pt')) and not recreate:
            self.labels = torch.load(file_path.replace('.csv', '_labels.pt'))
            self.data = torch.load(file_path.replace('.csv', '_data.pt'))
        else:
            self.labels, self.data = self.__load_from_csv(file_path)
            torch.save(self.labels, file_path.replace('.csv', '_labels.pt'))
            torch.save(self.data, file_path.replace('.csv', '_data.pt'))
    ...
    def __load_from_csv(self, csv_file):
        ...
        for _, row in grouped.iterrows():
            splitted_items = self.__split_up_evenly(row["tensor"], 200)
